...
static const char *my_tracker_settings_key_model_number =
    "mytracker_model_number";
...
MyTrackerDeviceDriver::MyTrackerDeviceDriver(unsigned int my_tracker_id) {
  ...
  vr::VRSettings()->GetString(my_tracker_main_settings_section,
                              my_tracker_settings_key_model_number,
                              model_number, sizeof(model_number));
  my_device_model_number_ = model_number;

  // Generate a unique serial number
  my_device_serial_number_ =
      my_device_model_number_ + std::to_string(my_tracker_id);

  DriverLog("My Controller Model Number: %s", my_device_model_number_.c_str());
  DriverLog("My Controller Serial Number: %s",
            my_device_serial_number_.c_str());
}
...
vr::EVRInitError MyTrackerDeviceDriver::Activate(uint32_t unObjectId) {
  ...
  vr::PropertyContainerHandle_t container =
      vr::VRProperties()->TrackedDeviceToPropertyContainer(my_device_index_);

  // Set standard properties
  vr::VRProperties()->SetStringProperty(container, vr::Prop_ModelNumber_String,
                                        my_device_model_number_.c_str());
  vr::VRProperties()->SetStringProperty(
      container, vr::Prop_InputProfilePath_String,
      "{simpletrackers}/input/mytracker_profile.json");

  // Set up input components (buttons/triggers)
  vr::VRDriverInput()->CreateBooleanComponent(
      container, "/input/grip/click", &input_handles_[MyComponent_grip_click]);
  vr::VRDriverInput()->CreateScalarComponent(
      container, "/input/trigger/value",
      &input_handles_[MyComponent_trigger_value], vr::VRScalarType_Absolute,
      vr::VRScalarUnits_NormalizedOneSided);
  vr::VRDriverInput()->CreateBooleanComponent(
      container, "/input/trigger/click",
      &input_handles_[MyComponent_trigger_click]);
  ...
  if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) {
    DriverLog("Failed to initialize Winsock. Error Code : %d",
              WSAGetLastError());
...
void MyTrackerDeviceDriver::ReceiveLoop() {
  ...
  while (isRunning) {
    ...
    int recv_len = recvfrom(udpSocket, (char *)&packet, sizeof(IMUPacket), 0,
    ...
    vr::VRServerDriverHost()->TrackedDevicePoseUpdated(
        my_device_index_, GetPose(), sizeof(vr::DriverPose_t));
  }
...
vr::DriverPose_t MyTrackerDeviceDriver::GetPose() {
  ...
  // Since we only have an IMU, we cannot track position changes (drift is too
  // high).
...
void MyTrackerDeviceDriver::Deactivate() {
  ...
  // Closing the socket forces recvfrom() to unblock and fail, exiting the
  // thread
...
void *
MyTrackerDeviceDriver::GetComponent(const char *pchComponentNameAndVersion) {
  return nullptr;
}

void MyTrackerDeviceDriver::DebugRequest(const char *pchRequest,
